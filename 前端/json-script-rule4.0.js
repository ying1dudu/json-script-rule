...
const jsr_add_args = [{                       //添加插件的启动参数例子
    ...
    defaults:{remark:'字段为空时默认的备注信息'}, //表示哪些字段设有默认值
    data:[{name:'王五',salary:'88'},{name:'赵六',salary:'10'}]         //要插入的数据数组，mysql数据库时需要每一个对象的属性个数相同，否则需要后台到application配置文件中配置edi.rule.open.replenishColumns为true
}]
const jsr_edit_args = [{                      //修改插件的启动参数例子
    ...
    type:'or',
    editAll:false,                            //是否在没有设置修改条件的情况下修改全部数据，默认为false
    eq:{},
    ne:{},
    li:{},
    nl:{},
...
const jsr_delete_args = [{                    //删除插件的启动参数例子
    ...
    deleteAll:false,                          //是否在没有设置删除条件的情况下删除全部数据，默认为false
    type:'or',
    eq:{},
    ne:{},
    li:{},
...
function jsr_build_get(jsr_get_args){
    ...
    if (jsr_get_args.groupShow){
        get.groupShow = jsr_get_args.groupShow;
    }
    if (!jsr_is_empty(jsr_get_args.relation)){
        get.relation.condition = {};
        get.relation.condition.matches = jsr_get_args.relation;
    }
...
function jsr_build_add(jsr_a_args){
    ...
    let add = {
        ...
        data:jsr_a_args.data
    };
    if (!jsr_is_empty(jsr_a_args.defaults)){
        add.defaults = jsr_a_args.defaults;
    }
    if (!jsr_is_empty(jsr_a_args.uuid)){
        add.uuid = jsr_a_args.uuid;
    }
    if (!jsr_is_empty(jsr_a_args.snowFlake)){
        add.snowFlake = jsr_a_args.snowFlake;
    }
...
function jsr_build_edit(jsr_e_args){
    ...
    if (jsr_e_args.editAll){
        edit.editAll = jsr_e_args.editAll;
    }
...
function jsr_build_delete(jsr_d_args){
    ...
    if (jsr_d_args.deleteAll){
        del.deleteAll = jsr_d_args.deleteAll;
    }
