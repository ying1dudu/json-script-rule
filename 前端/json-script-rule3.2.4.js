...
function jsr_build_condition(args,plugin){
    let matches = {};
    if (!jsr_is_empty(args.equal)){
        matches.equal = {};
        jsr_build_codition_type(matches.equal,args.equal);
    }
    if (!jsr_is_empty(args.notEqual)){
        matches.notEqual = {};
        jsr_build_codition_type(matches.notEqual,args.notEqual);
    }
    if (!jsr_is_empty(args.like)){
        matches.like = {};
        jsr_build_codition_type(matches.like,args.like,"like");
    }
    if (!jsr_is_empty(args.notLike)){
        matches.notLike = {};
        jsr_build_codition_type(matches.notLike,args.notLike,"like");
    }
    if (!jsr_is_empty(args.in)){
        matches.in = {};
        jsr_build_codition_type(matches.in,args.in);
    }
    if (!jsr_is_empty(args.notIn)){
        matches.notIn = {};
        jsr_build_codition_type(matches.notIn,args.notIn);
    }
    if (!jsr_is_empty(args.gt)){
        matches.gt = {};
        jsr_build_codition_type(matches.gt,args.gt);
    }
    if (!jsr_is_empty(args.gteq)){
        matches.gteq = {};
        jsr_build_codition_type(matches.gteq,args.gteq);
    }
    if (!jsr_is_empty(args.lt)){
        matches.lt = {};
        jsr_build_codition_type(matches.lt,args.lt);
    }
    if (!jsr_is_empty(args.lteq)){
        matches.lteq = {};
        jsr_build_codition_type(matches.lteq,args.lteq);
    }
